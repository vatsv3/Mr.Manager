...
export const ProfilePage: React.FC = () => {
  const { currentUser, isGuest, logout, players, getPlayerAggregatedStats, setEditPlayerId, isAdmin } = useApp();
  const { id } = useParams<{ id?: string }>();
  const navigate = useNavigate();

  const targetPlayer = id ? players.find(p => p.id === id) : (isGuest ? null : currentUser);
    ...
    return (
      <div className="p-8 text-center space-y-4 max-w-sm mx-auto">
        ...
        <div className="space-y-1">
          <h3 className="text-base font-bold text-zinc-100">Guest Spectator</h3>
          <p className="text-xs text-zinc-400 leading-relaxed">
            You are browsing in Guest Mode. Tap on any player from the Squad Roster to inspect their full radar ratings, goal tally, and match history.
          </p>
        </div>
        <div className="space-y-2 pt-2">
          <button
            onClick={() => navigate('/roster')}
            className="w-full text-xs font-semibold text-zinc-950 bg-zinc-100 hover:bg-white px-4 py-2.5 rounded-xl inline-flex items-center justify-center gap-1.5 transition shadow"
          >
            <ArrowLeft className="w-3.5 h-3.5" /> Browse Squad Roster
          </button>
          <button
            onClick={async () => {
              await logout();
            }}
            className="w-full text-xs font-semibold text-emerald-400 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 px-4 py-2.5 rounded-xl inline-flex items-center justify-center gap-1.5 transition"
          >
            Sign In with Player Account
          </button>
        </div>
      </div>
    );
